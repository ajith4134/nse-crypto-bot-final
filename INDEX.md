...
## `research/market-making/analyze_markout.py`
_Markout / adverse-selection analysis of recorded maker fills._
- **functions:** `load(path)`; `mid_at(bk, times, t)`; `analyse(books, trades)`; `report(res)`
- **imports:** bisect, collections, json, statistics, sys

## `research/market-making/collect_markout.py`
_Fill-toxicity (markout) collector for the market-making feasibility test._
- **imports:** asyncio, json, os, sys, time, websockets

## `research/market-making/entry_ab_report.py`
_Randomized taker-vs-maker ENTRY A/B — intention-to-treat readout._
- **functions:** `flat(p)`; `load_orders(db)`; `main(log_path, db)`
- **imports:** collections, json, math, sqlite3, statistics, sys, time

## `research/market-making/entry_fill_rate.py`
_Did our PASSIVE (limit-at-touch) entries actually fill?_
- **functions:** `flat(p)`; `load_decisions(path)`; `load_trades(db)`; `match(decisions, trades)`; `report(decisions)`
- **imports:** collections, json, sqlite3, statistics, sys, time

## `research/market-making/latency_test.py`
_Is our 162 ms round-trip latency survivable for passive quoting?_
- **functions:** `load_books(path)`; `analyse(bk)`
- **imports:** bisect, collections, json, statistics, sys

## `research/market-making/stale_quote_markout.py`
_What WE would earn -- not what the average maker earns._
- **functions:** `load(path)`; `run(books, trades)`
- **imports:** bisect, collections, json, statistics, sys
...
- **functions:** `_snap(bid, bq, ask, aq, bids, asks)`; `test_mid_field_persists_and_series_exports_of_mid(tmp_path, monkeypatch)`
...
## `tests/test_direction_ledger.py`
_Tests for the Direction Decision Ledger (owner ask: what data drove each direction)._
...
- **classes:** _Base, TestDayBucketsAndDecay, TestReliabilityChainDecayed, TestHorizonEmission, TestCostGate, TestSession2, TestHorizonSpecialized, TestCorrectDirectionNeverInverts, TestMagnitudeCap, TestCleanWindowRows, TestDeepScanFixes
...
- **classes:** _Iso, TestKillCriteria, TestFreshnessGate, TestStatus, TestPlaceOrderWiring, TestPhaseConditioner, TestStopChurnCooldown, TestDeadMarketFloor, TestLiquidityFloor, TestCounterTrendRefusal
...
- **classes:** MirrorPriceTest, CandleAggregationTest, LiquidationNormalizationTest
...
_tests/test_postmortem.py — Trade Post-Mortem & Excursion Engine (trading/brain/postmortem.py)._
- **classes:** PostmortemBase, TestFeatures, TestMining, TestExcursion, TestSignalAndExplain, TestGracefulFallback
...
## `trading/brain/direction_ledger.py`
_trading/brain/direction_ledger.py — the Direction Decision Ledger (owner ask 2026-07-13)._
- **functions:** `record(decision) -> None`; `recent(n) -> list[dict]`; `by_symbol(symbol) -> dict | None`; `summary() -> dict`
- **imports:** __future__, time
...
## `trading/brain/postmortem.py`
_trading/brain/postmortem.py — Trade Post-Mortem & Excursion Engine._
...
- **functions:** `_next_bar_close() -> float`; `_nse_open() -> bool`; `_mark(st, by, label) -> None`; `main() -> int`
- **imports:** __future__, datetime, os, sys, time
...
_trading/broker_sense/stealth.py — make the brain's browser look like a human's._
- **functions:** `enabled() -> bool`; `_script() -> str`; `apply(context) -> bool`
- **imports:** __future__, os
...
- **functions:** `enabled() -> bool`; `_f(name, default) -> float`; `_flat(symbol) -> str`; `ab_enabled() -> bool`; `_ab_arm(symbol, side) -> str | None`; `_drift_bps(flat) -> float | None`; `choose(symbol, side) -> dict`; `last_choice(symbol, side) -> dict | None`; `log_choice(symbol, side, choice) -> None`; `grade_fills(trades) -> int`; `status() -> dict`
- **imports:** __future__, hashlib, json, os, pathlib, time, trading
