# ai-scientist state snapshot — 20260713-123426

**Branch:** feat/trading-t8  ·  **deps:** 135  ·  **vendored OSS:** 37

## Module inventory (current capabilities)
trading:448, tests:208, nodes:59, .:42, core:17, cognition:16, memory:13, dashboard:8, tools:7, scratchpad:7, native:4, eval:2
...
`research/audits/independent-audit-20260713-054103.md`
...
## Recent commits
- b175a4d Per-market scorecard: crypto vs NSE kept distinct (audit #5)
- 6a0b917 Close R6 / R12 / R13 / R27 gaps (audit #4)
- 888f678 Add NSE-funnel kill-switch (owner: stop NSE trading funnel)
- 6b09751 Grow instruction library from strategies + honest genius-use denominator (audit #3)
- d63e97e Measure applied-instruction lift + enable the tilt in paper (audit #2)
- 2348809 Fix two dashboard honesty/perf bugs (audit #1)
- cb5d6fd Separate NSE and crypto in the brain + cross-process WM unification
- d0d4df9 Self-eval standing cron: trend genius-use / accumulation / parity over time (R23/R28)
- ee3eb33 Apply instructions in the decision path - closes evolve->apply->grade->promote
- 73ce6ab P2-2: AWM induction - mint instruction neurons from successful trajectories
- 23da63a Brain-OS OS-4: route consult/grade seams through the kernel syscall surface
- 6f749fe Fix: Upstox instrument-key leaking into OpenAlgo ("NSE_FO|51380 not found on NSE")
- d9dac03 Brain-OS OS-3 attention scheduler + stitch shipped features into the neuron web
- 0a89e0c UI-TARS last-resort: durable state-flag activation (live pickup, no restart)
- 00daac7 UI-TARS wired as SELECTIVE LAST-RESORT grounder (off the hot path)
