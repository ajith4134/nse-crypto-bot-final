# ai-scientist state snapshot — 20260717-075026

**Branch:** feat/trading-t8  ·  **deps:** 137  ·  **vendored OSS:** 37

## Module inventory (current capabilities)
trading:468, tests:229, nodes:59, .:42, core:17, cognition:16, memory:13, tools:9, dashboard:8, scratchpad:7, native:4, eval:2

## Current dependencies (replacement-scan targets)
`EMD-signal`, `EntropyHub`, `MFDFA`, `NRCLex`, `POT`, `PyWavelets`, `QuantLib`, `RestrictedPython`, `TA-Lib`, `adtk`, `aeon`, `ag-ui-protocol`, `alphalens-reloaded`, `antropy`, `arch`, `astropy`, `autogluon.tabular`, `autots`, `bayesian-changepoint-detection`, `catboost`, `causal-learn`, `chromadb`, `control`, `crepes`, `darts`, `deeptime`, `dowhy`, `econml`, `empyrical-reloaded`, `ewtpy`, `faster-whisper`, `feature-engine`, `filterpy`, `flaml`, `functime`, `gplearn`, `hdbscan`, `hmmlearn`, `hyppo`, `imodels`, `inferactively-pymdp`, `interpret`, `jax`, `jaxlib`, `kokoro-onnx`, `librosa`, `lifelines`, `lightgbm`, `lightgbmlss`, `mapie`, `meteostat`, `metric-learn`, `minisom`, `mlforecast`, `mljar-supervised`, `multitaper`, `nemoguardrails`, `netcal`, `networkx`, `neurokit2`

## Vendored OSS (reuse pool)
`ChartScanAI`, `RD-Agent`, `__pycache__`, `a_mem`, `alphagen`, `bocd`, `browser_use_src`, `candlestick_cnn`, `crypto_whale_watching`, `dfa`, `efficient_kan`, `finmem`, `freqtrade`, `frequi`, `funding-rate-arbitrage`, `gamma-scalping`, `generative_agents_memory`, `gplearn`, `granite_tsfm`, `hipporag`, `lag_llama`, `llama2_c`, `lob_deep_learning`, `lob_regime_scanner`, `microprice`, `muzero_general`, `nanogpt`, `noVNC`, `nse_data_dump`, `omniparser`, `reflexion`, `statistical-arbitrage-pairs-trading`, `tradingagents`, `uni2ts`, `upstox_proto`, `vaderSentiment`, `voyager`

## Latest independent audit
`research/audits/independent-audit-20260717-074900.md`
...
## Recent commits
- 6d108ac Video-derived direction upgrades: opening-range VP grammar + cross-symbol state + cycle null gate
- 5463975 Kill the SECOND inverter: confidently-wrong sources are retired, never flipped
- 1d2e0fb P0 closed + P1 Input Hunt: true-book capture verified consumed; of_mid = join-free labels
- 91c4ca7 B4: the promotion gate was a WALL — short trade returns were inverted; fixed at root
- 21a4277 B3 lens paper lane: nine orphaned lenses now trade under their own identity
- 2ff0c6d Land the concurrent session's tournament-cap + panel work (owner: commit everything)
- 14ac5d5 B1+B2 fix batch: the ensemble was part fake and the learning loops unwired — now honest
- d3bf8fd exit_reason gets its own field + a loop-period profiler (stop guessing at the cadence)
- 6f75790 THE cycle bottleneck: a full TradeJournal reload PER RECORDED ENTRY (found by py-spy)
- 03de7f5 Ledger measurement is now auditable + the 1h edge CONCENTRATES by clock phase
- d056141 CONFIRMED: pct_change contrarian at 15m/1h (-3.5σ/-2.5σ) and MOMENTUM at 4h (+2.4σ), n=2205 clean fixed-horizon
- 7700a76 MEASURED: pct_change is CONTRARIAN at our horizon (-3.4σ, n=2469) + tag the poisoned inverter era
- d9f9fe0 Kill the inverter that shorted everything + CONVENTIONS §16: direction must be EARNED
- 599909e debug-error evolves only on FAILURE: closing-step rule + the enforcement hook (unwired)
- 0878fa8 Remove the duplicate spot-short guard my previous edit left behind
