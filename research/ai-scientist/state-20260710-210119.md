# ai-scientist state snapshot — 20260710-210119

**Branch:** feat/trading-t8  ·  **deps:** 132  ·  **vendored OSS:** 35

## Module inventory (current capabilities)
trading:396, tests:150, nodes:59, .:42, core:17, cognition:16, memory:11, dashboard:8, tools:7, eval:2, native:2
...
- a87279f Profit-tailgate columns land on the UNIFIED trades table (NSE + crypto) + revive Cerebras provider
- 1db0b39 Options segment actually trades: segment-aware tradeability guard + honest entered + book-liquidity guard
- de480bd Brain Screen Mirror + eyes/executor wiring lands together + per-segment funnel executors
- 2cb4092 Invent-beyond #1/#3/#4/#6: grounded eyes + champion bandit + micro-policy + off-policy lanes
- d88743b Tailgate trail-rate on the trades table + hand skill-cache (#2) + lazy/cached free-eyes (#5)
- f441c22 CRITICAL: revive the Freqtrade worker loop (dead since the fork) + phantom-pair guard + budgeted mirrors
- 6b63b64 Trade-throughput root-cause fix (unlimited max finally flows) + Binance ⭐ Favorites Brain-Open mirror
- b2524d3 Ledger: invent-beyond #5 marked DONE with day-one live numbers
- 41086f2 Invent-beyond #5: LIVE autoresearch loop — driver daemon + Trading-Researcher panel + cross-process UI-candle snapshot
- b6d8f4f Doc-truth wrap-up of the 8-videos session: goal checklist ticked with delivering commits
- 2eb6078 #10 Human-UI: Binance+Upstox crawl parity + eyes-hand chain health check
- baac5e1 #16 invent-beyond: curiosity feature-discovery + active-inference crawl (owner-named)
