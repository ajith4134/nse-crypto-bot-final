# Independent connectivity audit — 20260713-150902

_Engine: **ast+networkx (fallback)** · modules: 841 · edges: 2850 · report-only._
...
- trading.crypto.freqtrade.micro_policy → trading.crypto.freqtrade.percoin_decider → trading.crypto.freqtrade.brain_executor → …
- trading.crypto.freqtrade.percoin_decider → trading.crypto.freqtrade.brain_executor → …
- trading.broker_sense.watchlist_study → trading.broker_sense.indicator_fusion → trading.evidence → trading.broker_sense.broker_features → trading.brain.boss → trading.broker_sense.account_watchlist → …
...
- trading.broker_sense.watchlist_study → trading.direction.micro_features → trading.direction.regime → trading.direction.truth_ledger → trading.broker_sense.broker_features → trading.brain.boss → trading.broker_sense.account_watchlist → …
...
- trading.broker_sense.watchlist_study → trading.direction.regime → trading.direction.truth_ledger → trading.broker_sense.broker_features → trading.brain.boss → trading.broker_sense.account_watchlist → …
...
- trading.strategy.direction_equation_deploy → trading.direction.mirror_gate → trading.direction.truth_ledger → trading.broker_sense.broker_features → trading.brain.boss → trading.online.controls → trading.online.live_loop → trading.broker_sense.stock_xray → trading.broker_sense.indicator_fusion → …
- trading.strategy.direction_equation_deploy → trading.direction.mirror_gate → trading.direction.truth_ledger → trading.broker_sense.broker_features → trading.brain.boss → trading.online.live_loop → trading.broker_sense.stock_xray → trading.broker_sense.indicator_fusion → …
- trading.strategy.direction_equation_deploy → trading.direction.truth_ledger → trading.broker_sense.broker_features → trading.brain.boss → trading.online.controls → trading.online.live_loop → trading.broker_sense.stock_xray → trading.broker_sense.indicator_fusion → …
- trading.strategy.direction_equation_deploy → trading.direction.truth_ledger → trading.broker_sense.broker_features → trading.brain.boss → trading.online.live_loop → trading.broker_sense.stock_xray → trading.broker_sense.indicator_fusion → …
- trading.strategy.direction_equation_deploy → trading.strategy.direction_equation → trading.broker_sense.indicator_fusion → …
- trading.online.controls → trading.online.live_loop → trading.broker_sense.stock_xray → trading.broker_sense.indicator_fusion → trading.evidence → trading.broker_sense.broker_features → trading.brain.boss → …
- trading.online.controls → trading.online.live_loop → trading.crypto.freqtrade_ingest → trading.broker_sense.broker_features → trading.brain.boss → …
- trading.online.controls → trading.online.live_loop → trading.crypto.freqtrade_ingest → trading.strategy.champion_bandit → trading.broker_sense.broker_features → trading.brain.boss → …
- trading.online.controls → trading.online.live_loop → trading.direction.dir_exit → trading.direction.micro_features → trading.direction.regime → trading.direction.truth_ledger → trading.broker_sense.broker_features → trading.brain.boss → …
- trading.online.controls → trading.online.live_loop → trading.direction.dir_exit → trading.direction.micro_features → trading.direction.truth_ledger → trading.broker_sense.broker_features → trading.brain.boss → …
- trading.online.controls → trading.online.live_loop → trading.direction.dir_exit → trading.direction.mirror_gate → trading.direction.truth_ledger → trading.broker_sense.broker_features → trading.brain.boss → …
- trading.online.controls → trading.online.live_loop → trading.direction.dir_exit → trading.direction.regime → trading.direction.truth_ledger → trading.broker_sense.broker_features → trading.brain.boss → …
- trading.online.controls → trading.online.live_loop → trading.direction.dir_exit → trading.direction.truth_ledger → trading.broker_sense.broker_features → trading.brain.boss → …
- trading.online.controls → trading.online.live_loop → trading.direction.mirror_gate → trading.direction.truth_ledger → trading.broker_sense.broker_features → trading.brain.boss → …
- trading.online.controls → trading.online.live_loop → trading.direction.truth_ledger → trading.broker_sense.broker_features → trading.brain.boss → …
...
- `trading.state` ← 220 importers
- `trading` ← 175 importers
- `trading.broker_sense` ← 93 importers
- `core.node_protocol` ← 88 importers
- `trading.strategy.freqtrade_strategy_base` ← 81 importers
- `trading.brain` ← 66 importers
- `core` ← 54 importers
- `core.llm` ← 30 importers
- `eval.golden` ← 28 importers
- `trading.direction` ← 28 importers
- `core.registry` ← 23 importers
- `memory.neurons` ← 23 importers
- `trading.broker_sense.sessions` ← 22 importers
- `trading.brain.mind_events` ← 21 importers
- `trading.broker_sense.ui_data` ← 21 importers
...
- `trading.openalgo_client` ← 18 importers
- `nodes` ← 17 importers
- `nodes.quant_nodes` ← 17 importers
- `trading.broker_sense.ui_market` ← 16 importers
- `trading.crypto.config` ← 16 importers

**Most coupled** (imports the most — split candidates):
- `dashboard.routes.trading_ext` → 71 deps
- `dashboard.routes.brain_ext` → 45 deps
- `trading.crypto.freqtrade.brain_executor` → 40 deps
- `trading.online.live_loop` → 40 deps
- `dashboard.server` → 35 deps
- `trading.broker_sense.funnel` → 30 deps
- `dashboard.routes.post_ext` → 28 deps
- `run_multi` → 28 deps
- `trading.broker_sense.run_funnel_loop` → 25 deps
- `trading.broker_sense.indicator_fusion` → 23 deps
...
- `trading.brain.learn_loop` → 17 deps
- `trading.crypto.freqtrade_ingest` → 16 deps
...
- Undocumented on disk: **1** (first 1 shown)
  - `.cortex_build_job.py`
- Stale index entries (in INDEX.md, not on disk): **526**
