...
{
  "refuted": false,
  "evidence": "CLAIM SURVIVES adversarial review. Attempted refutation on all 5 checklist axes; none landed.\n\n1) QUOTE SUPPORT \u2014 not an overreach. Full-text extraction of arXiv:2112.13213 (Cont, Cucuringu, Zhang, \"Price Impact of Order Flow Imbalance: Multi-level, Cross-sectional and Forecasting\") shows the paper's own numbered contribution #1 states the claim near-verbatim: \"We show that order flow at deeper levels of the limit order books provides additional explanatory power for price impact, when compared to the order flow imbalance at the best-level.\" The claim is weaker than what the source asserts.\n\n2) CONTRADICTING EVIDENCE \u2014 searched; found corroboration instead. Xu, Gould & Howison, \"Multi-Level Order-Flow Imbalance in a Limit Order Book\" (arXiv:1907.06230; published Market Microstructure and Liquidity, World Scientific) independently finds on 6 liquid Nasdaq stocks that \"the out-of-sample goodness-of-fit of the relationship improves with each additional price level,\" with ~65-75% improvement (large-tick) and ~15-30% (small-tick). Prior skepticism about deep levels was an OLS artifact: under Ridge, deep-level coefficients are strongly statistically significant. Cont et al. cite Xu et al. [56] approvingly. Two independent primary sources, same direction.\n\n3) SOURCE QUALITY \u2014 sufficient/high. Rama Cont is a leading microstructure authority; Cont et al. (2005/[17]) originated the OFI definition being extended. v1 Dec 2021 -> v4 Jun 2023; peer-published in Quantitative Finance 2023 (Taylor & Francis) as \"Cross-impact of order flow imbalance in equity markets.\" Not a press release, not a benchmark, not forum speculation.\n\n4) NOT OUTDATED \u2014 OFI/price-impact is a structural microstructure relationship, not a fast-moving-field claim; peer-reviewed publication is 2023.\n\nMATERIAL QUALIFICATIONS (do not refute the claim as worded, but constrain its use):\n(a) Cont et al. explicitly warn: \"as OFIs at deeper order book levels are included as predictors, the explained percentage of variance steadily increases in the in-sample tests. Nonetheless, the performance on out-of-sample data gets worse after a certain level of OFIs.\" They observe strong multicollinearity within the top 10 levels; the FIRST principal component explains >80% of total variance. Their remedy is an \"integrated OFI\" via PCA, which is \"superior to both best-level OFIs and multi-level OFIs, in both in-sample and out-of-sample.\" => RECORDING per-level OFI (what the claim asserts) is correct and is the prerequisite for integration; ENUMERATING 20 raw per-level OFIs as separate model features reproduces the exact in-sample-up/out-of-sample-down pathology that is already this project's failure mode (Sharpe gate r=-0.031, n=260).\n(b) DEPTH SCOPE: paper uses M=10 levels (\"Throughout the paper, we use the first M = 10 levels\"). Levels 11-20 are untested and not endorsed by this evidence. The claim's \"20-level\" framing exceeds the tested range; support extends to top ~10.\n(c) DOMAIN TRANSFER: both primary sources are Nasdaq EQUITIES, not crypto perps. Untested on perps; tick-size regime matters (deep-level contribution decays for small-tick names, is near-uniform for large-tick).\n(d) CONTEMPORANEOUS != PREDICTIVE: the supporting quote concerns contemporaneous price impact, which is not itself a tradable entry edge. The paper treats forecasting separately (lagged cross-asset OFIs improve future-return R2); do not import the contemporaneous R2 as expected entry alpha.\n\nENTRY-SNAPSHOT IMPLICATION (supported): snapshot per-level OFI for levels 1..10 (extendable to 20 as untested exploration), plus the level-wise depth normalizer, at entry timestamp \u2014 sufficient to reconstruct integrated/PCA OFI offline. Fit the PCA loadings on train-only data to avoid look-ahead.",
  "confidence": "high",
  "counterSource": "No contradicting source found. Closest thing to a counter is the source's OWN self-qualification (arXiv:2112.13213 full text): raw multi-level OFIs degrade out-of-sample beyond a certain depth due to strong multicollinearity across the top 10 levels, requiring PCA integration \u2014 this qualifies the MODELING step, not the claim that multi-level OFI carries power beyond best-level. Historical skepticism toward deep-level informativeness (referenced by Xu et al. arXiv:1907.06230) was shown to be an OLS-methodology artifact and reverses under Ridge, so it does not stand as a counter."
}
```
...
{
  "type": "started",
  "key": "v2:60dbcc8a70314ce275b5b9a89a0fdd4c024e1b9ac598d51569a02251736b726d",
  "agentId": "ad66ac6b22a0cd099"
}
```
...
```
{
  "refuted": true,
  "confidence": "high",
  "counterSource": "Primary source self-refutation: arXiv 2112.13213v1 Table 1 vs Table 3 and results text p.12; plus superseding v4 / Quantitative Finance published abstract (DOI 10.1080/14697688.2023.2236159, arxiv.org/abs/2112.13213 latest version)",
  "evidence": "REFUTED on three independent grounds; the strongest is that the paper's own data contradicts the sentence quoted.\n\n(1) THE IN-SAMPLE HALF IS FALSE BY THE PAPER'S OWN TABLES. Table 1 (in-sample adj-R2, contemporaneous price impact): PI[1]=71.16%, PI[5]=87.66%, PI[8]=89.04%, PI[10]=89.38%. Table 3 (integrated PCA OFI, PII): in-sample R2 = 87.14% \u2014 LOWER than the individual multi-level models PI[5] through PI[10]. The paper's own results text (p.12) states verbatim: \"even though the in-sample R2 of PII is lower than PI[m] (when m>=5), its performance in out-of-sample tests is better than PI[m] (for all 1<=m<=10). It demonstrates the ability of PCA to extract information from data and reduce the chance of overfitting.\" PCA integration is therefore an IN-SAMPLE SACRIFICE traded for OOS robustness \u2014 the precise opposite of \"outperforms both in-sample AND out-of-sample\" vs individual-level OFIs. The v1 abstract/intro sentence the claim relies on (\"integrated OFIs are superior to both best-level OFIs and multi-level OFIs, in both in-sample and out-of-sample tests\") is contradicted by the paper's own Table 1 vs Table 3. The claim is only true if \"individual-level OFIs\" is narrowed to best-level (level-1) OFI alone (87.14 > 71.16); against the multi-level individual OFIs it names, it is false in-sample. The OOS margin is also thin: PII 83.83% vs best multi-level PI[8] 83.16% = +0.67pp.\n\n(2) THE QUOTED LANGUAGE WAS WITHDRAWN UNDER PEER REVIEW \u2014 SOURCE IS SUPERSEDED. The claim cites v1 (25 Dec 2021, \"Price Impact of Order Flow Imbalance: Multi-level, Cross-sectional and Forecasting\"). It was superseded by v4 (13 Jun 2023) and published in Quantitative Finance as \"Cross-impact of order flow imbalance in equity markets\". The published/v4 abstract DELETES the PCA sentence entirely, retreating to: \"we propose a systematic approach for combining OFIs at the top levels of the limit order book into an integrated OFI variable which better explains price impact, compared to the best-level OFI.\" Both the multi-level comparison AND the in-sample/out-of-sample superiority language are gone, and the surviving comparison is only vs BEST-LEVEL OFI and only for \"explains price impact\" (contemporaneous). Quoting v1 for this claim quotes wording the authors removed.\n\n(3) THE OOS WIN IS CONTEMPORANEOUS (NON-TRADEABLE); THE FORECASTING OOS R2 IS NEGATIVE. The 83.83% OOS R2 is same-bucket OFI explaining same-bucket return \u2014 not a prediction, unusable for trade entry. In the actual one-minute-ahead forecasting models (Table 6), integrated OFI's OOS R2 = -17.62% (F-PII) vs -19.07% (F-PI[1]) \u2014 both worse than predicting the mean; lagged-return baselines reach -36.62%. The paper hedges: \"We remark that negative R2 values do not imply that the forecasts are economically meaningless.\" So for the entry-edge use case, integrated OFI has NO demonstrated positive OOS predictive power.\n\n(4) DOMAIN MISMATCH for the research question: US equities (Nasdaq, 1-minute buckets, Jan 2017\u2013Jan 2020, 30-min rolling refit), not crypto perps. A crypto order-flow study (Anastasopoulos & Gradojevic, EFMA 2025) further notes PCA-derived latent factors are not highly predictive of crypto returns.\n\nVERDICT: overreach/misread of a superseded preprint whose own results tables show the in-sample claim is backwards, and which conflates contemporaneous explanatory R2 with tradeable forecasting power. The defensible residual claim is narrower: \"PCA-integrated OFI gives slightly better OUT-OF-SAMPLE contemporaneous price-impact R2 than any fixed-depth multi-level OFI model, at the cost of in-sample R2, in US equities.\""
}
```
...
{
  "type": "started",
  "key": "v2:0cb0473345336a4e86cb2a39cb19c5147df1a4f55a0f09dbdb2549e206c61cd8",
  "agentId": "a90641a40bafae786"
}
```
...
{
  "refuted": false,
  "evidence": "CLAIM SURVIVES, but with material scope qualifications the claim's phrasing omits.\n\n1) SUPPORTED BY QUOTE \u2014 not an overreach on the empirical core. The v1 abstract states verbatim that \"cross-sectional OFIs significantly increase both in-sample and out-of-sample R\u00b2\" for predicting future returns. The claim also correctly preserves the paper's subtle two-sided result (cross-impact redundant CONTEMPORANEOUSLY, additive for FORECASTING) \u2014 the part most secondary summaries get wrong. \"Significantly\" is the source's own word, so the claim is not inflating it.\n\n2) SOURCE QUALITY EXCEEDS the claim's strength. Not a preprint-only result: v1 (arXiv 2112.13213, 25 Dec 2021, \"Price Impact of Order Flow Imbalance: Multi-level, Cross-sectional and Forecasting\") was superseded by v4 / \"Cross-Impact of Order Flow Imbalance in Equity Markets,\" peer-reviewed and published in Quantitative Finance 23(10), 2023, pp. 1373\u20131393, by Rama Cont, Mihai Cucuringu, Chao Zhang. Cont is a primary authority in market microstructure. The forecasting finding SURVIVED peer review \u2014 the published abstract retains: \"lagged cross-asset OFIs do improve the forecasting of future returns.\" Citing v1 rather than the published version is a citation-hygiene nit, not a refutation.\n\n3) NO CONTRADICTING EVIDENCE FOUND. Searches surfaced corroboration, not dispute. Independent crypto-specific work (\"Order Flow and Cryptocurrency Returns,\" EFMA 2025 / ScienceDirect S1386418126000029) reports \"strong out-of-sample economic value for portfolios sorted by ML models which condition on multiple international order flows\" \u2014 i.e. cross-sectional order flow carries predictive value in crypto specifically. arXiv 2602.00776 (\"Explainable Patterns in Cryptocurrency Microstructure\") finds OFI dominates mean-absolute-SHAP for short-horizon crypto returns.\n\n4) QUALIFICATIONS THE CLAIM UNDERSTATES (why this is not a blank check):\n- ASSET-CLASS TRANSFER: data is NASDAQ LOBSTER ITCH, top-100 S&P 500 by mkt cap, 2017-01-01 to 2019-12-31. US EQUITIES, not crypto perps. The claim's operational inference (\"per-coin snapshot should include BTC/ETH OFI\") is an EXTRAPOLATION not tested by this source; it is separately supported by the crypto papers above, not by 2112.13213.\n- HORIZON: the result is a ~1-minute-ahead effect that \"mainly manifests at short-term horizons and decays rapidly in time,\" essentially gone by ~10-20 min. This is the load-bearing constraint for our stack: with 500ms L2 the feature is capturable, but a 5m-bar entry gate sits at/past the decay tail. Cross-OFI must be snapshotted from the 500ms book at entry instant, not derived from 5m bars.\n- MAGNITUDE: the CONTEMPORANEOUS numbers I confirmed (own-OFI 83.83% vs cross-impact 83.62% R\u00b2) show cross terms ADD NOTHING contemporaneously \u2014 consistent with the claim's concession. LASSO selects only ~8.29% of possible cross terms for integrated-OFI contemporaneous impact, i.e. cross-impact is sparse. Forecasting-table R\u00b2 figures I retrieved (PI ~3.45% vs CI ~3.89% at 1 min, decaying to <0.3% by 30 min) came from an LLM-summarized PDF fetch that earlier failed to find magnitudes at all, so I treat those exact digits as LOW-TRUST/approximate; the tandfonline full text 403'd. The QUALITATIVE shape (small base R\u00b2, small incremental cross-term gain, rapid decay) is confirmed by multiple independent sources.\n- ECONOMIC vs STATISTICAL: \"significantly\" = statistical significance on a ~3-4% R\u00b2 base with a sub-1pp incremental gain. Economic gains after transaction costs are modest. Statistically real \u2260 tradeable, which is directly relevant to a project whose current gate already shows zero backtest-Sharpe\u2192profit correlation.\n\nVERDICT: refuted=false. Not unsupported, not contradicted, not outdated, not marketing, primary+peer-reviewed source. Recommend the synthesis carry the claim as: \"snapshot lagged BTC/ETH (and cluster-peer) OFI at entry from the 500ms book, as a SHORT-HORIZON (~1-10 min) feature with rapidly decaying, small incremental R\u00b2; equities-proven, crypto-corroborated by separate sources; do not expect it to survive at 5m+ horizons or to be economically large after costs.\"",
  "confidence": "medium",
  "counterSource": "https://www.tandfonline.com/doi/abs/10.1080/14697688.2023.2236159 (published successor, Quantitative Finance 23(10) 2023 \u2014 retains and QUALIFIES the finding: gains are short-horizon and decay rapidly; contemporaneous cross-impact adds nothing, own-OFI 83.83% vs cross-impact 83.62% R\u00b2); https://arxiv.org/html/2112.13213v4 (v4 supersedes the cited v1); corroborating (not contradicting) crypto evidence: https://www.sciencedirect.com/science/article/pii/S1386418126000029 and https://arxiv.org/html/2602.00776v1"
}
```
...
{
  "type": "started",
  "key": "v2:53a1845e141858ed9909c4e8774b7abef89e9ed26f34c638cbfb6057eb2f6172",
  "agentId": "aeb8d94b0c76ed3bb"
}
```
...
```
{
  "refuted": true,
  "confidence": "high",
  "counterSource": "arXiv:2112.13213v1 full text (Table 1, Table 3, Table 6, and Section 4 \"Performance of Integrated OFIs\") \u2014 i.e. the paper's own body contradicts its own abstract; plus arXiv:2112.13213v4 published abstract (retitled \"Cross-Impact of Order Flow Imbalance in Equity Markets\", Cont/Cucuringu/Zhang, Oxford)",
  "evidence": "REFUTED on three independent grounds, all from the primary source itself.\n\n(1) INTERNALLY CONTRADICTED ON THE IN-SAMPLE HALF. The claim's supporting quote is the v1 abstract, but the paper's body says the opposite: \"even though the in-sample R2 of PII is lower than PI[m] (when m >= 5), its performance in out-of-sample tests is better than PI[m] (for all 1<=m<=10).\" Numbers confirm: Table 3 PII in-sample R2 = 87.14%; Table 1 PI[5]=87.66%, PI[8]=89.04%, PI[9]=89.24%, PI[10]=89.38% \u2014 all HIGHER than PII in-sample. So \"outperforms individual-level OFIs both in-sample AND out-of-sample\" is false against the multi-level baselines. The abstract overstates; the claim inherits the overstatement.\n\n(2) SCOPE MISREAD \u2014 \"OUT-OF-SAMPLE\" IS CONTEMPORANEOUS FIT, NOT PREDICTION. The paper defines: \"contemporaneous returns denote the returns that materialize over the same bucket of time as the OFI.\" The headline OOS R2=83.83% comes from fitting on a past 30-min window and scoring on the next 30-min window, but the regression still explains SAME-BUCKET returns from SAME-BUCKET OFI. This is untradeable at entry and is not evidence of forecasting edge \u2014 exactly the distinction that matters for an entry-snapshot feature.\n\n(3) THE ACTUAL FORECASTING TEST FAILS. Table 6 (predicting 1-min-ahead returns from LAGGED OFIs, rolling 30-min fit): integrated OFI F-PII out-of-sample R2 = -17.62%; best-level F-PI[1] OOS R2 = -19.07%. Both NEGATIVE (worse than the mean). PCA integration only narrows the loss by ~1.45pp. The authors concede: \"We remark that negative R2 values do not imply that the forecasts are economically meaningless\" \u2014 an admission that predictive OOS R2 did not survive.\n\n(4) VERSION DRIFT / QUOTE IS FROM A SUPERSEDED PREPRINT. The quote is v1 (25 Dec 2021). The published v4 (2023) is retitled \"Cross-Impact of Order Flow Imbalance in Equity Markets\" and its abstract DROPS both the PCA framing and the \"superior results in both in-sample and out-of-sample\" language, softening to \"an integrated OFI variable which better explains price impact, compared to the best-level OFI\" \u2014 i.e. the authors themselves retreated to the best-level-only comparison and to \"explains\" (contemporaneous), removing the specific sentence the claim rests on.\n\n(5) ASSET-CLASS EXTRAPOLATION. Data is Nasdaq equities, 2017-2019, 1-minute buckets, top-10 LOB levels. Zero evidence for crypto perps (500ms L2, 24/7, no auction open/close \u2014 the paper even drops the first/last 30 min of each session, a structure crypto lacks).\n\nSTEELMAN ACKNOWLEDGED (does not rescue the claim): PII does beat the single best-level PI[1] both in-sample (87.14 vs 71.16) and out-of-sample (83.83 vs 64.64), and PII does beat every multi-level PI[m] out-of-sample (83.83 vs peak 83.16 at PI[8]). That narrower finding \u2014 PCA reduces overfitting relative to stuffing 10 collinear per-level OFIs into one regression \u2014 is genuine and well-evidenced. But it is (a) contemporaneous explanatory power, (b) versus a single-level baseline, and (c) reverses in-sample against multi-level. The claim as worded, and as it would be used to justify snapshotting PCA-OFI as an entry predictor, is not supported."
}
```
...
{
  "refuted": true,
  "evidence": "REFUTED BY THE PRIMARY SOURCE'S OWN TABLES. I extracted the full PDF (arxiv.org/pdf/2112.13213v1, 40pp, Cont/Cucuringu/Zhang, Dec 2021).\n\n1) IN-SAMPLE HALF IS FALSE. Table 1 (multi-level OFI, in-sample adj-R2 %): PI[1]=71.16, PI[5]=87.66, PI[6]=88.30, PI[7]=88.74, PI[8]=89.04, PI[9]=89.24, PI[10]=89.38. Table 3 (integrated PCA OFI): in-sample R2 = 87.14, OOS = 83.83. PII (87.14) is LOWER in-sample than PI[5] through PI[10]. The paper states this verbatim on p.12: \"Compared with Table 1, we observe that even though the in-sample R2 of PII is lower than PI[m] (when m>= 5), its performance in out-of-sample tests is better than PI[m](for all 1<=m<=10).\" This directly contradicts the paper's own intro (p.2): \"integrated OFIs are superior to both best-level OFIs and multi-level OFIs, in both in-sample and out-of-sample tests.\" The abstract quote the claim rests on is contradicted by the body. It is also mathematically impossible: PII is a rank-1 restriction of the nested PI[10] (paper notes \"PI[m] is a nested model of PI[m+1]\"), so it CANNOT beat the unrestricted model in-sample. Integrated OFI beats only the BEST-LEVEL (PI[1]) model in both IS and OOS; against the multi-level/per-level comparator implied by \"dimensionality reduction,\" it wins OOS ONLY (83.83 vs best PI[8]=83.16, +0.67pp).\n\n2) THE CLAIM'S GLOSS INVERTS THE MECHANISM. \"the dimensionality reduction is not an in-sample artifact\" misstates the finding. PCA's entire benefit IS out-of-sample regularization, PAID FOR with an in-sample R2 cost (89.38 -> 87.14). Paper's own words: \"It demonstrates the ability of PCA to extract information from data and reduce the chance of overfitting.\"\n\n3) FATAL FOR TRADE-ENTRY USE (the research question's purpose). The 87.14/83.83 figures are CONTEMPORANEOUS price impact \u2014 same-bucket returns (\"contemporaneous returns denote the returns that materialize over the same bucket of time as the OFI\"), i.e. a nowcast/identity, not a forecast. The paper's own forecasting section (Sec 5.3, 1-min ahead) reports integrated-OFI forecasting: F-PII In-Sample R2 = 0.26%, Out-Of-Sample R2 = -17.62% (NEGATIVE); best-level F-PI OOS = -19.07%. So as an entry predictor the integrated OFI has NEGATIVE OOS R2 in the cited paper. Only cross-sectional/LASSO terms (F-CI OOS -11.81%) reduce the loss, still negative.\n\n4) SCOPE MISMATCH. Data = Nasdaq ITCH via LOBSTER, ~100 S&P500 large-cap EQUITIES, 2017-01-01 to 2019-12-31, minutely buckets. Zero crypto/perp evidence; no replication found on crypto perps.\n\nSource quality is high (Rama Cont; published Quantitative Finance 23(10), 2023) and not outdated \u2014 the paper is fine. The CLAIM is an overreach that parrots an abstract sentence the paper's own Tables 1/3 and p.12 text refute.",
  "counterSource": "https://arxiv.org/pdf/2112.13213v1 (p.12 text + Table 1 vs Table 3; Sec 5.3 forecasting table) \u2014 the primary source contradicts its own abstract. Corroborating: https://www.emergentmind.com/topics/order-flow-imbalance-ofi (\"outperforms those using multi-level OFIs in out-of-sample tests\" \u2014 OOS only). Published version: https://www.tandfonline.com/doi/abs/10.1080/14697688.2023.2236159",
  "confidence": "high"
}
```
...
{
  "type": "started",
  "key": "v2:8e9d9b67cbd663d6a1f7fa4f1d6b48ba599c3ff53efe54ff39f9eeb25cae82aa",
  "agentId": "a0da6487053dda492"
}
```
...
{
  "type": "started",
  "key": "v2:240743551647026c37801941c5a02c66b3ca95bed7e6e9631c54304e5e5bc62b",
  "agentId": "aaa2938f3989359d7"
}
```
...
{
  "refuted": true,
  "evidence": "SOURCE IS REAL AND QUOTE IS VERBATIM \u2014 but the claim overreaches on the decisive point.\n\nPaper: arXiv:2607.09230v1, \"When Does Order Flow Matter? State-Dependent L2 Liquidity-State Transitions in Crypto Futures\" (Joohyoung Jeon, Korea University, submitted 2026-07-10). Abstract confirms the quote word-for-word: \"Order flow adds further value only when layered on top of the L2 state model, not as a replacement.\"\n\nREFUTATION 1 \u2014 WRONG TARGET; the paper explicitly disclaims the inference the claim draws. Abstract: \"We define a supervised discrete L2 liquidity-state transition task, distinct from latent-regime detection and price-direction prediction.\" The predicted variable is a calm/mixed/stressed LIQUIDITY REGIME label (terciles of relative spread, top-20 depth, top-20 imbalance) at 1min/5min. The claim's operative sentence \u2014 \"This implies an entry snapshot must capture the L2 state variables first, with order-flow features layered on\" \u2014 transfers a liquidity-regime-forecasting hierarchy onto trade-ENTRY EDGE (direction/profit), the exact transfer the authors rule out. WebFetch of the abs page: \"Price Direction Claims: The authors explicitly exclude this.\" Nothing in the paper concerns trade entry (\"Trade Entry Claims: None mentioned\").\n\nREFUTATION 2 \u2014 CONTRADICTED FOR THE TARGET WE ACTUALLY CARE ABOUT. Cont, Kukanov & Stoikov (arXiv:1011.6402, \"The Price Impact of Order Book Events\") establish order flow imbalance as a FIRST-ORDER DIRECT driver of short-horizon price changes \u2014 a robust near-linear OFI\u2192return relation, stable across a large cross-section and across intraday sampling scales, with slope inversely proportional to depth. For direction/return prediction, order flow is a primary predictor and L2 depth enters as the SCALING term \u2014 i.e. the opposite hierarchy to the one the claim asserts as universal. The claim's \"never as a replacement\" is not a property of L2-vs-order-flow generally; it is an artifact of choosing liquidity state as the label.\n\nREFUTATION 3 \u2014 THE CLAIM'S \"DOMINANT/NEVER\" FRAMING INVERTS THE PAPER'S OWN NEGATIVE RESULT. Abstract: \"This value is not robustly cross-symbol: for ETH it is present across calm, mixed, and stressed regimes and largest under stressed pre-event liquidity, whereas BTC shows only isolated five-minute passes and no regime that clears at both horizons.\" So order flow's contribution is UNESTABLISHED for half the sample. The claim launders \"we could not establish an order-flow effect on BTC\" into a universal design law (\"never as a replacement\").\n\nREFUTATION 4 \u2014 SCOPE MISMATCH + NEAR-TAUTOLOGY. Sample = 2 symbols (BTCUSDT, ETHUSDT) inside MACRO-EVENT WINDOWS with matched controls; our context is 200 coins, general entry timing, no event conditioning. The abstract also concedes the baseline is uninformed: \"we use event timing but not the event's label content, so pre-event state competes against an uninformed within-window baseline.\" And \"pre-event state predicts post-event state\" over 1-5min is largely the persistence/autocorrelation of spread/depth/imbalance \u2014 highly serially correlated quantities \u2014 not evidence of a tradeable edge. Notably, interpretable logits over continuous L2 features \"fail to improve on\" the coarse tercile baseline, which argues the result is dominated by state persistence.\n\nREFUTATION 5 \u2014 SOURCE STRENGTH BELOW CLAIM STRENGTH. Single-author, un-peer-reviewed preprint, 6 days old (2026-07-10), zero independent replication, being used to justify a universal architectural mandate (\"dominant\", \"never\", \"must\") outside its declared task. The paper itself only claims to \"motivate a state-first design principle for market microstructure models\" \u2014 a motivation for microstructure/execution modelling, not a proven law for entry-edge feature ordering.\n\nSALVAGEABLE CORE: as a narrow statement \u2014 \"within macro-event windows on BTC/ETH, pre-event L2 tercile state is the first-order predictor of post-event LIQUIDITY REGIME, and order flow only adds incremental value on top (ETH only)\" \u2014 the claim is supported. Useful for sizing/execution/abstention gating (predicting whether the book will be stressed when you exit), NOT for ranking entry-edge predictors. The snapshot recommendation is still worth adopting for an unrelated reason: spread, depth-20, and imbalance-20 at entry are cheap, non-leaky, and needed for cost modelling \u2014 but this paper does not license \"L2 first, order flow layered\" as the entry-edge hierarchy.",
  "counterSource": "https://arxiv.org/abs/1011.6402 (Cont, Kukanov & Stoikov, \"The Price Impact of Order Book Events\" \u2014 OFI as first-order direct driver of short-horizon price changes, with depth as the scaling term, inverting the claimed hierarchy for direction targets); plus the source paper's own abstract at https://arxiv.org/abs/2607.09230 which self-disclaims price-direction applicability and reports the order-flow effect as not robustly cross-symbol (BTC fails at both horizons).",
  "confidence": "high"
}
```
...
{
  "refuted": true,
  "evidence": "SOURCE IS REAL, QUOTE IS VERBATIM, BUT CLAIM OVERREACHES VIA TARGET SWAP.\n\nVerified: arXiv 2607.09230v1 = \"When Does Order Flow Matter? State-Dependent L2 Liquidity-State Transitions in Crypto Futures\" (Joohyoung Jeon, Korea University, submitted 2026-07-10). Binance BTCUSDT/ETHUSDT futures 2023\u2013mid-2026, top-20 L2. Supporting quote is accurate.\n\nREFUTATION 1 \u2014 TARGET SWAP (fatal for this research question). The paper's dependent variable is discrete LIQUIDITY REGIME (calm/mixed/stressed), not entry edge. Paper states verbatim: \"Our dependent variable is not price but the post-event liquidity state\" and the task is \"distinct from latent-regime detection and price-direction prediction.\" It makes NO claims about trade entry, direction, or profitability. The claim's operative second sentence \u2014 \"This implies an entry snapshot must capture the L2 state variables first, with order-flow features layered on\" \u2014 is the reviewer's inference, not the paper's, and jumps from a liquidity target to a profit target with zero bridging evidence. The research question asks about trade-ENTRY edge; the paper answers a different question.\n\nREFUTATION 2 \u2014 CONTRADICTED ON THE TARGET THAT MATTERS. Cont, Kukanov & Stoikov, \"The Price Impact of Order Book Events\" (arXiv:1011.6402; SSRN 1712822): for PRICE changes, order flow imbalance is the dominant driver \u2014 a near-linear OFI\u2192price relation with slope inversely proportional to market depth. Static depth enters only as a scaling DENOMINATOR, not as the primary predictive object. That is the exact INVERSE of the claimed \"L2 state first, order flow as overlay\" hierarchy. Results shown robust to seasonality and stable across time scales and stocks. So the claimed hierarchy is target-specific and flips when the target becomes direction/price.\n\nREFUTATION 3 \u2014 SELF-REFUTING SCOPE. (a) Event windows only: \"The macro-event calendar enters only by locating the windows\" \u2014 findings are conditional on scheduled macro announcements, not general trading. (b) n=2 symbols (BTC, ETH) \u2014 no cross-sectional evidence for a 200-coin perp universe. (c) The paper itself disclaims the overlay half: \"This value is not robustly cross-symbol: for ETH it is present... whereas BTC shows only isolated five-minute passes.\" A finding non-robust across 2 symbols cannot support the absolute \"never as a replacement.\" (d) Effect size modest: 0.034\u20130.045 over the marginal baseline. (e) Cadence 1-min/5-min, not the 500ms book available here.\n\nREFUTATION 4 \u2014 NEAR-TAUTOLOGY. Liquidity regime is strongly autocorrelated, so \"pre-event state dominates post-event state\" is close to a persistence baseline, which the authors concede is methodological (\"baseline and evaluation protocol\"). Persistence does NOT transfer to a direction target: this project's own 2026-07-16 measurement rebuild found persistence corr \u22120.214 (Mirror Gate retired, MIRROR_GATE=0) \u2014 direct local counter-evidence to the transfer.\n\nVERDICT: source quality is adequate (primary, current, arXiv preprint \u2014 though not peer-reviewed and single-author). The paper supports the claim's FIRST clause about liquidity regime, but the claim as written asserts an entry-snapshot design principle that the paper explicitly does not make and that the price-impact literature inverts. Refuted as stated. Salvageable narrow version: \"for predicting post-event LIQUIDITY REGIME in macro-event windows on BTC/ETH, pre-event L2 state terciles beat order-flow-only models\" \u2014 that version is supported, but it does not license L2-state-first ordering of an entry snapshot.",
  "confidence": "high",
  "counterSource": "https://arxiv.org/abs/1011.6402 (Cont, Kukanov & Stoikov, \"The Price Impact of Order Book Events\" \u2014 OFI is the dominant driver of short-horizon price changes, depth enters only as inverse slope scaling, inverting the claimed hierarchy for price/direction targets); plus the primary source's own scope disclaimers at https://arxiv.org/abs/2607.09230 (\"Our dependent variable is not price but the post-event liquidity state\"; \"not robustly cross-symbol\")"
}
```
...
{
  "type": "started",
  "key": "v2:bbc2247d43ed0d6ad26ddf39c27cc81252ab15313ec12b274717ace59962eb29",
  "agentId": "ace6f9a7f8ad0ff63"
}
```
...
{
  "type": "started",
  "key": "v2:2eb09b5a7118c889229549a960dc55c77e2f881f35191f880d84b413c4fff870",
  "agentId": "aec19f2e225be1817"
}
```
...
{
  "refuted": true,
  "evidence": "SOURCE IS REAL AND THE QUOTE IS ACCURATE \u2014 but the claim overreaches on the only part that matters for this research question (trade-entry edge).\n\n1) TASK MISMATCH (fatal). arXiv:2607.09230v1 (Jeon, subm. 2026-07-10, \"When Does Order Flow Matter? State-Dependent L2 Liquidity-State Transitions in Crypto Futures\") predicts a 3-class POST-EVENT LIQUIDITY REGIME label: \"Yi,h = Si,h^post, S \u2208 {calm, mixed, stressed}\" over [t, t+h), h \u2208 {1m, 5m}. It is not a price/entry-edge study. The paper's own limitations state VERBATIM: \"This is a prediction study of liquidity-state transitions at a one-minute cadence... whether pre-event state is similarly first-order for a price-direction target on the same panel is UNTESTED.\" The paper explicitly frames its task as \"distinct from ... price-direction prediction\" and makes \"no claims about price prediction or execution systems.\" The claim's operative sentence \u2014 \"This implies an entry snapshot must capture the L2 state variables first, with order-flow features layered on\" \u2014 is precisely the inference the authors refuse to draw. Unsupported by the quote.\n\n2) DIRECTLY CONTRADICTED ON THE DIRECTION TASK. Cont, Kukanov & Stoikov, \"The Price Impact of Order Book Events\" (arXiv:1011.6402 / J. Financial Econometrics 12(1):47) find short-horizon price changes are MAINLY DRIVEN BY ORDER FLOW IMBALANCE, with depth acting as an inverse SLOPE SCALER (impact coefficient \u221d 1/depth), not as the first-order predictor. That is the inverse ordering of the claim (\"order flow only as an overlay, never a replacement\") for the entry/direction target. Cf. also \"Price Impact of Generalized Order Flow Imbalance\" (arXiv:2112.02947).\n\n3) EFFECT SIZES DON'T SUPPORT \"DOMINANT\". Paper's own deltas (joint NLL+Brier gains): coarse tercile state vs marginal = +0.034 (1m) / +0.045 (5m); nonlinear L2-SHAPE vs coarse state = +0.044 / +0.060 \u2014 i.e. the discretized-tercile state the claim names as \"dominant\" is BEATEN by the continuous L2 shape it discards. Order-flow overlay = +0.010 pooled, and BTC-only = +0.001 / +0.003 (essentially nil); the overlay result is carried almost entirely by ETH (+0.020/+0.016). n=2 symbols.\n\n4) SCOPE/GENERALIZATION. Sample = Binance BTCUSDT + ETHUSDT only, 2023\u2013mid-2026, event-window-centred (18,631 event windows + 28,882 matched controls, 9,773 scheduled events). Our context is 200 perps, continuous entries, not macro-event windows. A 2-symbol event-window result cannot carry \"dominant predictor\" weight cross-sectionally.\n\n5) PERSISTENCE CONFOUND. Reported stressed\u2192stressed transition prob up to 0.456; \"pre-event state predicts post-event state\" is substantially liquidity autocorrelation. Predicting a persistent state variable from its own lag says little about which fields carry EDGE at entry.\n\nBottom line: the citation supports \"L2 state > order flow FOR PREDICTING LIQUIDITY REGIME on BTC/ETH in event windows.\" It does NOT support the claim's stated implication for entry-snapshot design, which the paper calls untested and which the primary price-impact literature contradicts.",
  "counterSource": "https://arxiv.org/abs/2607.09230 (paper's own limitations: \"whether pre-event state is similarly first-order for a price-direction target on the same panel is untested\"); https://arxiv.org/abs/1011.6402 \u2014 Cont, Kukanov & Stoikov, \"The Price Impact of Order Book Events\" (OFI is the primary driver of short-horizon price changes; depth is a 1/depth slope scaler); https://arxiv.org/pdf/2112.02947 \u2014 \"The Price Impact of Generalized Order Flow Imbalance\"",
  "confidence": "high"
}
```
...
{
  "refuted": true,
  "evidence": "TARGET SUBSTITUTION \u2014 the numbers are real, the inference is not.\n\n1. NUMBERS CHECK OUT (not the problem). arXiv:2607.09230v1, \"When Does Order Flow Matter? State-Dependent L2 Liquidity-State Transitions in Crypto Futures\" (Joohyoung Jeon, Korea University, 2026-07-10) does report ETH order-flow increment +0.004 (calm) / +0.020 (mixed) / +0.038 (stressed) at 1-min, and +0.004/+0.015/+0.030 at 5-min. The supporting quote is verbatim. Source is a legitimate primary preprint, current (6 days old), not marketing.\n\n2. FATAL: THE PAPER IS NOT PREDICTING PRICE. The prediction target is *L2 liquidity-state transitions* \u2014 which liquidity configuration the book moves into. The abstract EXPLICITLY disclaims the very thing the claim uses it for: the task is \"distinct from latent-regime detection and **price-direction prediction**.\" The paper makes zero claims about returns, direction, or profitability. The claim silently recasts \"order flow helps predict the next book state\" into \"order-flow features are trustworthy [for trade entry].\" A trade-entry edge model predicts price direction/magnitude. The +0.038 gain says nothing about that. This is the exact error the research question's own Section (6) warns about \u2014 and worse, it is the same failure mode already burning this project (in-sample gate with -0.031 correlation to profit): a metric that improves while profit does not.\n\n3. THE PAPER'S OWN BTC RESULT UNDERMINES GENERALIZATION. BTC shows NO regime-robust effect \u2014 only 1 of 6 cells clears its flow-shuffle null, and no regime clears at both horizons. So of the 2 assets tested, the monotonic pattern appears in exactly 1. The claim quietly drops the 50% failure rate. Extrapolating an n=1-asset pattern to a 200-coin perp universe is unsupported; if anything BTC/ETH are the two most liquid, least representative perps in that universe.\n\n4. \"MONOTONIC\" IS 3 POINTS ON AN ORDERED FACTOR. Three ordered bins ascending has a 1-in-6 chance under random ordering. \"Monotonic\" here is a description of 3 numbers, not a demonstrated functional law. Regime cells clear a 90% (not 95%) event-cluster interval \u2014 a weak bar. BH correction at q=0.05 and 9,301 ETH windows are genuine strengths, but they establish the liquidity-state result, not the entry-edge one.\n\n5. WHAT SURVIVES. Narrow version \u2014 \"for ETH, order flow adds incrementally more to *L2 liquidity-state transition* prediction under stress, as an overlay atop L2 features, never standalone\" \u2014 is supported. The claim as written (regime field must be snapshotted to gate when order-flow features are *trustworthy* for entry) does not follow from this source. Snapshotting a regime/liquidity-state field at entry may still be justified on other grounds (well-established regime-dependence in independent work, e.g. order-flow informativeness varying by regime in Springer 2025 fee-structure study and ScienceDirect \"Order flow and cryptocurrency returns\"), but THIS paper cannot carry that recommendation, and its specific +0.004/+0.020/+0.038 figures must not be cited as entry-edge evidence.",
  "counterSource": "https://arxiv.org/abs/2607.09230 (the primary source itself \u2014 abstract explicitly excludes price-direction prediction, and its BTC arm fails to replicate the regime effect: only 1 of 6 cells clears null)",
  "confidence": "high"
}
```
...
{
  "type": "started",
  "key": "v2:efe2294d0cfc2cdd9070ac8ceed625149fab71a78fbc56e9dd2f53c5eb3ae2a2",
  "agentId": "a704d8264955a3927"
}
```
...
{
  "type": "started",
  "key": "v2:0198e3bc4a5e8aeacf6254171c28c2e543ca7ec0bd63e1ea7f25743e1cf04cca",
  "agentId": "a6845fc74dcebd994"
}
```
...
{
  "refuted": true,
  "evidence": "REFUTED ON SCOPE OVERREACH \u2014 the quote is verbatim and the source is real, but the claim transplants it onto a task the paper explicitly excludes.\n\n1) SOURCE VERIFIED, QUOTE VERIFIED. arXiv:2607.09230v1, \"When Does Order Flow Matter? State-Dependent L2 Liquidity-State Transitions in Crypto Futures\", Joohyoung Jeon (Korea University), 2026-07-10. Section 5.3 verbatim: \"The increment rises monotonically from calm to mixed to stressed, reaching 0.004, 0.020, and 0.038 at the one-minute horizon and 0.004, 0.015, and 0.030 at the five-minute horizon.\" So the numbers are not fabricated.\n\n2) FATAL: WRONG TARGET. The claim is offered as guidance for a \"crypto perp trade-entry edge model\" (i.e. direction/return prediction). The paper's own abstract explicitly fences this off: \"We define a supervised discrete L2 liquidity-state transition task, distinct from latent-regime detection AND PRICE-DIRECTION PREDICTION.\" The 0.004/0.020/0.038 increments are order flow's incremental value for classifying POST-EVENT LIQUIDITY REGIME \u2014 not for predicting price direction or trade P&L. Nothing in the paper licenses \"order-flow features are [more] trustworthy [for entry edge] in stress.\"\n\n3) \"RISES MONOTONICALLY WITH STRESS REGIME\" IS STATED AS A GENERAL LAW; THE PAPER SAYS THE OPPOSITE ABOUT GENERALITY. Abstract: \"This value is not robustly cross-symbol: for ETH it is present across calm, mixed, and stressed regimes and largest under stressed pre-event liquidity, whereas BTC SHOWS ONLY ISOLATED FIVE-MINUTE PASSES AND NO REGIME THAT CLEARS AT BOTH HORIZONS.\" n=2 assets; 1 of 2 fails outright. The claim quietly drops the ETH-only qualifier from the headline assertion (\"order flow's incremental value ... rises monotonically with stress regime\") and only reinstates it as an example.\n\n4) MONOTONICITY IS 3 ORDERED POINTS ON ONE ASSET. Under a null, 3 points fall in a monotone order with probability 1/3. With BTC as the only replication attempt and it failing, \"monotonic\" here is an observation, not an established relationship.\n\n5) SCOPE RESTRICTION DROPPED. The entire analysis lives INSIDE MACRO-EVENT WINDOWS with matched non-event controls (\"The macro-event calendar enters only by locating the windows\"). The claim generalizes to all entries at all times. Our own context (200 coins, continuous 500ms L2, no event-window conditioning) is outside the paper's evaluated regime.\n\n6) EFFECT SIZE IS TINY AND UNIT-LESS AS QUOTED. +0.038 in an unspecified metric (abstract does not name AUC/accuracy/log-loss), on the best regime of the better of two assets, on a liquidity-classification task. This cannot carry a \"MUST be snapshotted at entry\" imperative.\n\n7) SOURCE STRENGTH vs CLAIM STRENGTH. Single-author, ~6-day-old, non-peer-reviewed preprint, 2 symbols. Adequate for \"one preprint reports X on a liquidity-transition task\"; inadequate for a prescriptive engineering mandate on a direction/entry model.\n\nVERDICT: refuted. The correctly-scoped, survivable version would be: \"One 2026 preprint (BTC/ETH only, macro-event windows, LIQUIDITY-STATE-TRANSITION target, not direction) finds order flow's incremental value over an L2-state baseline is ETH-only and largest under stressed pre-event liquidity; BTC shows none. This is weak, indirect motivation \u2014 not evidence \u2014 for snapshotting a liquidity-state field at entry.\" The claim as written is an overreach on target, on cross-asset generality, and on scope.",
  "confidence": "high",
  "counterSource": "https://arxiv.org/abs/2607.09230 \u2014 the paper's own abstract is the primary counter-source, on three points: (a) target is \"distinct from ... price-direction prediction\"; (b) \"This value is not robustly cross-symbol ... BTC shows only isolated five-minute passes and no regime that clears at both horizons\"; (c) analysis confined to macro-event windows (\"The macro-event calendar enters only by locating the windows and supplying matched non-event controls\")."
}
```
...
{
  "type": "started",
  "key": "v2:64e82083171e29c461bcc573809343186cc961175c9ffca01cf1d8f5faa9886e",
  "agentId": "a7531c70cc2e5ea95"
}
```
...
{
  "refuted": true,
  "evidence": "Source is REAL and quote is VERBATIM, but the claim is an overreach on target, scope, and actionability.\n\nVERIFIED: arXiv:2607.09230v1 = \"When Does Order Flow Matter? State-Dependent L2 Liquidity-State Transitions in Crypto Futures\", Joohyoung Jeon (Korea University), 2026-07-10. Sec 5.3 verbatim: \"The increment rises monotonically from calm to mixed to stressed, reaching 0.004, 0.020, and 0.038 at the one-minute horizon.\" All six ETH regime cells clear a flow-shuffle null with 90% event-cluster intervals above zero.\n\nREFUTATION 1 \u2014 TARGET SUBSTITUTION (fatal). The paper's dependent variable is NOT price/direction: \"Our dependent variable is not price but the post-event liquidity state of the book, a discrete calm, mixed, or stressed regime\" (Y_{i,h} = S^post \u2208 {calm,mixed,stressed}, built from relative spread, top-20 depth, top-20 imbalance terciles). The claim recasts this as order flow's \"incremental predictive value\" for a TRADE-ENTRY EDGE model. Predicting whether the book will be stressed in 60s is a different object from predicting direction. The 0.004/0.020/0.038 figures do not speak to \"when order-flow features are trustworthy\" for entry.\n\nREFUTATION 2 \u2014 CONCLUSION EXPLICITLY DISCLAIMED BY SOURCE. Paper: \"This is a prediction study of liquidity-state transitions at a one-minute cadence, not a trading or execution study, and reports no policy, profit, or simulator result.\" The paper makes NO recommendation to snapshot a regime field at entry. The \"must be snapshotted at entry\" is the claim's own inference.\n\nREFUTATION 3 \u2014 GENERALIZATION ON n=2. Claim states a general law (\"Order flow's incremental predictive value is state-dependent and rises monotonically with stress regime\") but BTC, the only other asset studied, shows NO establishable order-flow contribution (+0.001 at 1m, +0.003 at 5m, not separated from null); only 1 of 6 BTC regime cells clears (5m calm) and the stressed regime shows no effect. Authors concede: \"the two-asset scope is a genuine limit and the ETH-versus-BTC asymmetry is a two-point observation rather than a population statement.\" Cannot support \"must\" for a 200-coin perp universe.\n\nREFUTATION 4 \u2014 UNITS OVERSOLD. Increments are negative-log-likelihood (with Brier secondary) gains vs a shuffle null, not accuracy/edge/Sharpe. Presenting +0.004 (calm) as an \"order-flow gain\" implies decision-relevant magnitude the metric does not carry; +0.004 sits in the same range the same paper declares non-establishable for BTC (+0.001/+0.003).\n\nREFUTATION 5 \u2014 PROVENANCE THIN FOR A \"MUST\". Single-author, v1, non-peer-reviewed preprint, 6 days old (2026-07-10), no replication. Corroborating literature also qualifies universality: Makarov & Schoar (2020) find a significant NEGATIVE order-flow/next-day-return relation for bitcoin; tick size modulates OFI predictive power.\n\nVERDICT: the narrow literal sentence about ETH liquidity-state prediction is supported; the claim built on it (order flow -> entry edge, therefore snapshot regime at entry) is not supported by the cited quote and is expressly outside the paper's stated scope.",
  "confidence": "high",
  "counterSource": "https://arxiv.org/html/2607.09230v1 (the primary source itself: \"our dependent variable is not price but the post-event liquidity state of the book\"; \"not a trading or execution study, and reports no policy, profit, or simulator result\"; \"the ETH-versus-BTC asymmetry is a two-point observation rather than a population statement\"; BTC order-flow +0.001/+0.003 not separated from null). Secondary qualification: Makarov & Schoar (2020) negative order-flow/next-day-return relation for bitcoin, via https://www.sciencedirect.com/science/article/pii/S1386418126000029"
}
```
...
{
  "type": "started",
  "key": "v2:a4148b31cbd516bd31f6df151ad652f4fde0e16a390f757058c546d9c582f116",
  "agentId": "afe11e5c672a79518"
}
```
