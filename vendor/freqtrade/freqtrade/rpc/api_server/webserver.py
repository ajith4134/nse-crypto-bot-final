...
class ApiServer(RPCHandler):
    ...
    _rpc: RPC
    _has_rpc: bool = False
    # mlnb: multi-segment fork — ONE ApiServer can serve several FreqtradeBot RPCs, keyed
    # by segment name (futures/spot/option/prediction). `_rpc`/`_has_rpc` stay pointing at
    # the first-registered segment so upstream single-bot code paths work unchanged.
    _rpcs: dict[str, RPC] = {}
    _config: Config = {}
    ...
    def __init__(self, config: Config, standalone: bool = False) -> None:
        if self.__initialized and (standalone or self._standalone):
            ...
            return
        # mlnb: multi-segment fork — bots 2..n construct ApiServer while the uvicorn server
        # of bot 1 is already LIVE in this process; they must only attach their RPC handler,
        # not spawn a second server on the same port. Keep bot 1's config (it owns the
        # api_server block). After cleanup() (_server reset to None) a re-init proceeds,
        # preserving upstream RELOAD_CONFIG behavior.
        if self.__initialized and getattr(self, "_server", None) and not standalone:
            return
        ApiServer._config = config
    ...
    def add_rpc_handler(self, rpc: RPC, segment: str = "default"):  # mlnb: per-segment RPCs
        """
        Attach rpc handler
        """
        # mlnb: one RPC per segment; raise only if THAT segment is already attached
        # (preserves the upstream "attach twice" error in single-bot mode). A NAMED
        # segment re-attaching (bot reload) replaces its handler instead of raising.
        if segment in ApiServer._rpcs:
            if segment == "default":
                # This should not happen assuming we didn't mess up.
                raise OperationalException("RPC Handler already attached.")
            logger.info(f"Replacing RPC handler for segment '{segment}' (bot reload).")
            ApiServer._rpcs.pop(segment)
        ApiServer._rpcs[segment] = rpc
        if not ApiServer._has_rpc:
            # mlnb: first-registered segment doubles as the legacy/default `_rpc`.
            ApiServer._rpc = rpc
    ...
    def cleanup(self) -> None:
        ...
        ApiServer._rpcs = {}  # mlnb: drop all segment RPCs
        del ApiServer._rpc
        ApiBG.exchanges = {}
        ApiBG.jobs = {}
        if self._server and not self._standalone:
            ...
            self._server = None  # mlnb: allow a clean re-init after RELOAD_CONFIG
    ...
    def configure_app(self, app: FastAPI, config):
        ...
        from freqtrade.rpc.api_server.api_mlnb import router as api_mlnb  # mlnb E2
        ...
        # mlnb E2: read-only brain-state endpoints (feed/funnel/tailgate/xray) — same
        # auth as the rest of the API, no trading-mode gate (state files are process-
        # independent, so the webserver mode can serve them too).
        app.include_router(
            api_mlnb,
            prefix="/api/v1",
            tags=["mlnb"],
