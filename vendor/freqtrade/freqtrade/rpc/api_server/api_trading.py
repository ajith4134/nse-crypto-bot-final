...
def status(request: Request, rpc: RPC = Depends(get_rpc), segment: str | None = Query(None)):
    # mlnb: multi-segment fork — with NO segment requested, aggregate open trades across
    # ALL segment bots, each serialized by ITS OWN RPC/exchange (another bot's exchange
    # cannot price a foreign pair, e.g. a Deribit option through the Binance bot).
    # 2026-07-10: the X-Freqtrade-Segment HEADER now scopes this endpoint too — it used to
    # be query-param-only here, so header callers silently got the all-segment union
    # (the "spot opens nothing" misdiagnosis).
    from freqtrade.persistence.segment_context import segment_scope
    from freqtrade.rpc.api_server.deps import _requested_segment
    from freqtrade.rpc.api_server.mlnb_sidecar import enrich_trades
    from freqtrade.rpc.api_server.webserver import ApiServer

    segment = segment or _requested_segment(request)
    if segment is None and len(ApiServer._rpcs) > 1:
        out = []
        for seg_name, seg_rpc in ApiServer._rpcs.items():
            try:
                with segment_scope(None if seg_name == "default" else seg_name):
                    out.extend(seg_rpc._rpc_trade_status())
            except RPCException:
                pass
        return enrich_trades(out, ApiServer._config)
    try:
        with segment_scope(None if segment in (None, "default") else segment):
            return enrich_trades(rpc._rpc_trade_status(), ApiServer._config)
    except RPCException:
        return []
...
def trades(
    ...
    hist = rpc._rpc_trade_history(limit, offset=offset, order_by_id=order_by_id)
    if isinstance(hist, dict):
        enrich_trades(hist.get("trades") or [], ApiServer._config)  # mlnb E2: native fields
    return hist
...
def trade(tradeid: int = 0, rpc: RPC = Depends(get_rpc)):
    ...
    try:
        return enrich_trades(rpc._rpc_trade_status([tradeid]), ApiServer._config)[0]
    except (RPCException, KeyError, IndexError):
