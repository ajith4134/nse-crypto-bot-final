...
class TradeSchema(BaseModel):
    ...
    bot_segment: str | None = None  # mlnb: owning segment in the multi-segment fork
    # mlnb E2: brain-native fields served SAME-ORIGIN from the state sidecars
    # (rpc/api_server/mlnb_sidecar.py) — the live tailgate ratchet + entry-time decision
    # facts, so FreqUI needs no cross-origin overlay for them.
    tg_locked_pct: float | None = None
    tg_peak_pct: float | None = None
    tg_trail_dist: float | None = None
    strategy_label: str | None = None
    brain_pred: str | None = None
    timeframe: int
