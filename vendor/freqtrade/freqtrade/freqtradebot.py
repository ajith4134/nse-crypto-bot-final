...
class FreqtradeBot(LoggingMixin):
    ...
    def exit_positions(self, trades: list[Trade]) -> int:
        ...
        for trade in trades:
            ...
            # mlnb divergence (2026-07-07): catch EVERYTHING per-trade. Upstream lets a
            # non-DependencyException (e.g. ccxt.BadSymbol from an unpriceable phantom
            # pair) propagate, aborting this loop for ALL remaining trades — one phantom
            # trade froze max/min_rate tracking + strategy exits for every open position
            # for 2 days. One sick trade must never halt management of the healthy ones.
            except Exception as exception:  # noqa: BLE001
                logger.warning(
                    f"Unable to manage trade {trade.pair} (non-dependency error): {exception!r}"
                )
