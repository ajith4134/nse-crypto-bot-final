...
export function createBotSubStore(botId: string, botName: string) {
  ...
  const useBotStore = defineStore(botId, () => {
    ...
    async function getPairCandles(payload: PairCandlePayload) {
      if (payload.pair && payload.timeframe) {
        ...
        try {
          ...
          // Live /pair_candles only serves the bot's analyzed (strategy) timeframe — for any OTHER
          // timeframe it returns 0 candles ("No data available"). Freqtrade's /pair_history is a
          // webserver-only endpoint (503 "Bot is not in the correct state" while trading), so it
          // can't help here. Instead fall back to the MLNetworkBrain dashboard's /api/trading/candles
          // (ccxt OHLCV, works for any timeframe in any mode), converting it to the PairHistory shape.
          if (result && result.length === 0) {
            try {
              const st = await (await fetch(`${import.meta.env.BASE_URL}mlnb_status.json?t=${Date.now()}`)).json();
              const dash = String(st?.dashboard_url || '').replace(/\/$/, '');
              if (dash) {
                // full pair as-held: the dash endpoint routes spot symbols to the spot
                // pool and futures notation to the swap pool (futures-only listings
                // like NATGAS have no spot market to strip down to)
                const cr = await fetch(
                  `${dash}/api/trading/candles?symbol=${encodeURIComponent(payload.pair)}&market=CRYPTO&tf=${encodeURIComponent(payload.timeframe)}`,
                );
                const candles = (await cr.json())?.candles || [];
                if (candles.length) {
                  const data = candles.map((c: { time: number; open: number; high: number; low: number; close: number; volume: number }) => [
                    c.time * 1000, c.open, c.high, c.low, c.close, c.volume,
                  ]);
                  const m = /^(\d+)([mhdw])$/.exec(payload.timeframe);
                  const unit = { m: 60_000, h: 3_600_000, d: 86_400_000, w: 604_800_000 };
                  const tfMs = m ? Number(m[1]) * (unit[m[2] as keyof typeof unit] || 60_000) : 300_000;
                  result = {
                    strategy: botState.value?.strategy || '',
                    pair: payload.pair,
                    ...
                    buy_signals: 0,
                    sell_signals: 0,
                    last_analyzed: data[data.length - 1][0],
                    data_start_ts: data[0][0],
                    data_start: new Date(data[0][0]).toISOString(),
                    data_stop_ts: data[data.length - 1][0],
                    data_stop: new Date(data[data.length - 1][0]).toISOString(),
                  } as PairHistory;
                }
              }
            } catch {
              /* dashboard candles unavailable — leave empty (chart shows "No data available") */
            }
          }
          if (result) {
            candleData.value = {
    ...
    // mlnb Brain Cockpit: generic reader for the fork's /mlnb/* brain-state endpoints
    // (feed / funnel / tailgate / xray). Same-origin, same auth as every other call;
    // null on failure so panels render an honest empty state.
    async function getMlnbState(path: string) {
      try {
        const { data } = await api.get(`/mlnb/${path}`);
        return data;
      } catch (error) {
        console.error(error);
        return null;
