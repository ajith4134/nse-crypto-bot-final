...
def main() -> int:
    ...
    bx = BrainExecutor(client=cli, learner=learn)  # universe = bot's live whitelist (futures format)
    print(f"[brain-loop] start: interval={interval}s allow_live={allow_live} "
          f"learn_every={learn.interval}s", flush=True)
    while True:
        t0 = time.monotonic()
        try:
            res = bx.run_once(allow_live=allow_live)
            cnt = cli._client().count() or {}
            # show which strategy the brain CHOSE for each coin it entered (per-coin attribution)
            ...
            print(f"[brain-loop] {time.strftime('%H:%M:%S')} entered={res['entered']} "
            ...
            print(f"[brain-loop] cycle error: {e!r}", flush=True)
